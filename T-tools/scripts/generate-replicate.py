...
Examples:
    python generate-replicate.py O-output/02-weekly-post-galilee-living-land
    python generate-replicate.py 02-weekly-post-galilee-living-land
    python generate-replicate.py 02-weekly-post-galilee-living-land --video
    python generate-replicate.py --all
    python generate-replicate.py --list
...
def generate_images(images, output_dir, model="black-forest-labs/flux-1.1-pro", force=False):
...
def generate_video(prompt, output_dir, model="minimax/hailuo-02", force=False):
...
def process_project(brief_path, generate_video_flag=False, use_schnell=False, force=False):
    """Process a single project: generate images and optionally video."""
    image_model = "black-forest-labs/flux-schnell" if use_schnell else "black-forest-labs/flux-1.1-pro"
    ...
    results = generate_images(images, output_dir, model=image_model, force=force)

    # Optionally generate video from the hero prompt
    if generate_video_flag:
        ...
        if hero_images:
            ...
            video_result = generate_video(video_prompt, output_dir, force=force)
...
def main():
    if len(sys.argv) < 2:
        print("Pilgrim Prayers - Image & Video Generator (Replicate)")
        print()
        print("Usage:")
        ...
        print("  python generate-replicate.py --all --video")
        print("  python generate-replicate.py --list")
        print()
        print("Options:")
        print("  --all       Generate for ALL projects with a visual-brief.md")
        print("  --video     Also generate a short video from the hero image prompt")
        print("  --schnell   Use Flux Schnell (fast, cheap) instead of Flux Pro")
        print("  --force     Regenerate even if files already exist")
        print("  --list      List all projects and their media status")
        print()
        print("Examples:")
        print("  python generate-replicate.py O-output/02-weekly-post-galilee-living-land")
        print("  python generate-replicate.py 02-weekly-post-galilee-living-land")
        print("  python generate-replicate.py 02-weekly-post-galilee-living-land --video")
        print("  python generate-replicate.py --all")
        print("  python generate-replicate.py --all --video --schnell")
        sys.exit(1)

    generate_video_flag = "--video" in sys.argv
    use_schnell = "--schnell" in sys.argv
    ...
    if "--all" in sys.argv:
        ...
        print(f"Pilgrim Prayers - Image & Video Generator (Replicate)")
        ...
        for brief in briefs:
            ...
            results = process_project(
                brief,
                generate_video_flag=generate_video_flag,
